		...
		switch($path)
		{
			...
			case '/add_cat':
				if(isset($_REQUEST['submit']))
				{
                    ...
                    $file=$_FILES['file']['name'];
                    $path='../carbook-master/UPLOAD/CARADV/'.$file;
                    $tmp_file=$_FILES['file']['tmp_name'];
                    move_uploaded_file($tmp_file,$path);
				...
				case'/status':
					...
					case'/editcat':
						if(isset($_REQUEST['edit_category_id'])){
							$category_id=$_REQUEST['edit_category_id'];
							$where=array("category_id"=>$category_id);
							$res=$this->select_where('category',$where);
							$fetch=$res->fetch_object();
                            $old_img=$fetch->file;
							if(isset($_REQUEST['save'])){
								...
								if($_FILES['file']['size']>0){
									...
									$path='../carbook-master/UPLOAD/CARADV/'.$file;
									$tmp_file=$_FILES['file']['tmp_name'];
									$arr=array("category_name"=>$category_name,"file"=>$file,"updated_dt"=>$updated_dt);
									$res=$this->update('category',$arr,$where);
									move_uploaded_file($tmp_file,$path);
									// echo"<pre>";
									// 	print_r($arr);
									// 	exit;
									if($res){
										// unlink('../carbook-master/UPLOAD/CARADV/'.$old_img);
									...
									$arr=array("category_name"=>$category_name,"updated_dt"=>$updated_dt);
									$res=$this->update('category',$arr,$where);
									echo"
										<script>
										alert('update sucess');
                     ...
                     case'/editcaradv':
						...
						if(isset($_REQUEST['edit_adv_id']))
						{
							$adv_id=$_REQUEST['edit_adv_id'];
							$where=array("adv_id"=>$adv_id);
							...
							if(isset($_REQUEST['save'])){
								...
								if($_FILES['name']['size']>0){
								   $file=$_FILES['file']['name'];
								   $path='../carbook-master/upload/caradv';
								   $tmp_file=$_FILES['file']['tmp_name'];
								   move_uploaded_file($tmp_file,$path);
								   $arr=array("owner_name"=>$owner_name,"car_name"=>$car_name,"vehical_number"=>$vehical_number,"mobile"=>$mobile,"charge"=>$charge,"driver"=>$driver,"deposite"=>$deposite,"terms_condition"=>$terms_condition,"location_id"=>$location_id,"category_id"=>$category_id,"file"=>$file,"updated_dt"=>$updated_dt);
								   $res=$this->update('caradv',$arr,$where);
								   ...
								   if($res){
									...
									<script>
									alert('update sucess');
									window.location='manageCar_%20Adv';
									</script>
									";
								   }
									...
									$arr=array("owner_name"=>$owner_name,"car_name"=>$car_name,"vehical_number"=>$vehical_number,"mobile"=>$mobile,"charge"=>$charge,"driver"=>$driver,"deposite"=>$deposite,"terms_condition"=>$terms_condition,"location_id"=>$location_id,"category_id"=>$category_id,"updated_dt"=>$updated_dt);
								   $res=$this->update('caradv',$arr,$where);
								   echo"<script>
								    alert('upadte sucess');
									window.location='manageCar_%20Adv';
					...
					case '/editemp':
						if(isset($_REQUEST['edit_employee_id']))
						{
							...
							if(isset($_REQUEST['submit']))
							{
								...
								{
									$file=$_FILES['file']['name'];
									$path='../admin/upload/employee/'.$file;
									$tmp_file=$_FILES['file']['tmp_name'];
									move_uploaded_file($tmp_file,$path);
